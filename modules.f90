	...
	integer, parameter :: powXexp=1, hiim = 2, euclid_spectr = 3 , halpha = 4
    ...
    Type TRedWin
	    ...
	    logical :: counts_potential = .false. !terms in potentials at source
	...
	elseif (Win%dNdz == hiim) then
		!HI IM: fit to T_HI(z) from SIMFAST
		counts_background_z= -0.02485*z**2+0.2396*z+0.0576
	elseif (Win%dNdz == halpha) then
		counts_background_z= (1+z)**1.17/(1+1.37e-3*(1+z)**6.61)
    ...
    function temperature_HI(Win, z)
    !this function is only relevant for the evolution bias of the HI, Halpha and Lyman alpha

    Type(TRedWin) :: Win
    real(dl), intent(in) :: z
    real(dl) temperature_HI
	
	temperature_HI= -0.02485*z**2+0.2396*z+0.0576

    end function temperature_HI
    ...
    type CAMBparams
		...
		real(dl)  :: eps_WL, eps_GR, eps_IM, eps_gal, eps_ISW !marginalization parameters
...
500     if (L< lmax_extrap_highl) &
    ...
    do i=2,nthermo
        ...
        do RW_i = 1, num_redshiftwindows
            ...
            if (a > 1d-4) then
                ...
                if  (RedWin%kind == window_lensing .or.  RedWin%kind == window_counts .and. RedWin%DoRedshiftLensing) then
    ...
    do RW_i = 1, num_redshiftwindows
        ...
        if (Redshift_w(RW_i)%kind == window_lensing .or. &
            Redshift_w(RW_i)%kind == window_counts .and. Redshift_w(RW_i)%DoRedshiftLensing) then
    ...
    do j=jstart, TimeSteps%npoints
        ...
        do i = 1, num_redshiftwindows
            ...
            elseif ( RedWin%kind == window_counts) then
                ...
                if (RedWin%counts_evolve) then
					
					if (tau < CP%tau0 -0.1) then
						if (RedWin%dNdz == hiim .or. RedWin%dNdz == halpha) then
							back_count_tmp(j,i) =  temperature_HI(RedWin, 1/a-1)
							!for HIIM counts_background_z is T_HI, for Halpha we use THI to determine the evolution of n_HI
							RedWin%comoving_density_ev(j) = back_count_tmp(j,i)*adot
						else
