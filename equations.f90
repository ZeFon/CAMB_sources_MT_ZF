        ...
        if (tau>tau_start_redshiftwindows .or. plot_evolve) then
            ...
            do w_ix = 1, num_redshiftwindows
                ...
                elseif (W%kind == window_counts) then
                    ...
                    if (CP%tau0-tau > 0.1_dl .and. W%counts_radial) then
                        chi =  CP%tau0-tau
                    ...
                    if (W%Dofnlder) then
                        counts_dfnl_source= W%wing(j)*clxc*(3*(W%bias - 1)*(CP%omegab + CP%omegac)*1.686  &
                             *CP%h0**2/(2.99792458e5_dl**2*dgrho/grho/CP%SCtransferTOT))
                        !Newtonian gauge count density; bias assumed to be on synchronous gauge CDM density
						!ze:correction to the bias due to fnl included - Ref_change: Stefano Camera
						!fnl and normalisation are input parameters
                    else
                        counts_dfnl_source= 0
                    endif
					
                    if (W%Dobder) then
                        counts_db_source= W%wing(j)*clxc!
						!if fNL!=0 then multiply by *(1 + 3*(CP%omegab + CP%omegac)*1.686*CP%h0**2*CP%fNL/(2.99792458e5_dl**2*dgrho/grho/CP%SCtransferTOT))
                    else
                        counts_db_source= 0
                    endif
